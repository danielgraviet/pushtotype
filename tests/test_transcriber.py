"""Tests for whisperflow.transcriber — mocked so CI works without GPU/model."""
...
def test_transcriber_init_uses_cpu_when_no_cuda():
    ...
    with patch("whisperflow.transcriber.WhisperModel", return_value=fake_model) as mock_model_cls:
...
def test_transcribe_returns_string():
    ...
    with patch("whisperflow.transcriber.WhisperModel", return_value=fake_model):
        from whisperflow.transcriber import Transcriber
